...
\acrodef{AP}[AP]{Average Precision at Rank 1000}
...
\acrodef{BoWE}[BoWE]{Bag-of-Word-Embeddings}
...
\acrodef{CLSM}[CLSM]{Convolutional Latent Semantic Model}
...
\acrodef{Conv-KNRM}[Conv-KNRM]{Convolutional Kernel-based Neural Ranking Model}
\acrodef{Co-PACRR}[Co-PACRR]{A Context-Aware Neural IR Model for Ad-hoc Retrieval}
...
\acrodef{DRMM}[DRMM]{Deep Relevance Matching Model}
\acrodef{DRIVER}[DRIVER]{Digital Repository Infrastructure Vision for European Research}
\acrodef{DSSM}[DSSM]{Deep Structured Semantic Model}
...
\acrodef{ECIR}[ECIR]{European Conference on Information Retrieval}
...
\acrodef{GLangModel}[GLM]{Generalized Language Model}
...
\acrodef{HINT}[HiNT]{HIerarchical Neural maTching model}
...
\acrodef{K-NRM}[K-NRM]{Kernel based Neural Ranking Model}
...
\acrodef{LSTM-DSSM}[LSTM-DSSM]{LSTM Deep Structured Semantic Model}
...
\acrodef{MAP}[MAP]{Mean Average Precision at Rank 1000}
...
\acrodef{nDCG@20}[nDCG@20]{Normalized Discounted Cumulated Gain at Cutoff 20}
\acrodef{nDCG@100}[nDCG@100]{Normalized Discounted Cumulated Gain at Cutoff 100}
\acrodef{NESTOR}[NESTOR]{NEsted SeTs for Object hieRarchies}
\acrodef{NeuIR}[NeuIR]{Neural Information Retrieval}
...
\acrodef{NVSM}[NVSM]{Neural Vector Space Model}
...
\acrodef{OLAP}[OLAP]{On-Line Analytical Processing}
...
\acrodef{P@10}[P@10]{Precision at Cutoff 10}
\acrodef{P@20}[P@20]{Precision at Cutoff 20}
\acrodef{P2P}[P2P]{Peer-To-Peer}
\acrodef{PA}[PA]{Performance Analysis}
\acrodef{PAMT}[PAMT]{Pool-Assessment Management Tool}
\acrodef{PASM}[PASM]{Pool-Assessment Storing Manager}
\acrodef{PACRR}[PACRR]{A Position-Aware Neural IR Model for Relevance Matching}
...
\acrodef{PL2}[PL2]{Poisson estimation for randomness using Laplace succession for normalisation}
...
\acrodef{SIGIR}[SIGIR]{ACM SIGIR Conference on Research \& Development in Information Retrieval}
...
\acrodef{SR}[SR]{Status Register}
\acrodef{SRNN}[SRNN]{Spatial Recurrent Neural Network}
