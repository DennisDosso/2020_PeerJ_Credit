...
\rone{Given a database instance $I$, the query $Q$, and the result $Q(D)$, consider on tuple $t$ in this output. 
 Lineage is the simplest among the forms of provenance. It has been defined in different ways \cite{CheneyProvSurvey}, but it can be thought as the set of all the tuples are are used in some way by the query to produce the output tuple, i.e., the ones that are somehow \emph{relevant} to its generation.}
...
It takes the form of a polynomial, called \emph{provenance polynomial}, where the variables are taken from the set $X$ of identifiers of the tuples (provided that each tuple in $I$ has an identifier) and the coefficients are drew from the set of natural numbers $\mathbb{N}$. \rtwo{This semiring therefore is commonly referred as $\mathbb{N}[X]$ in the literature.}
% SOME FORMALISM
...
Computing the causality of tuples is NP-complete in general~\cite{EiterL02}, but \citet{MeliouGMS11} proved that the causality of conjunctive queries may be determined in PTIME.
