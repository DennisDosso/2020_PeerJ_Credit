...
\rtwo{Also, we present the notions of Causality and Responsibility}. 

\subsection{Lineage}
Lineage was first introduced by \citet{lineageCui}; in this work we rely on the definition given by \citet{CheneyProvSurvey}.
\rone{Lineage is the simplest among the forms of provenance. It has been defined in different ways \cite{CheneyProvSurvey}, but it can be thought as the set of all the tuples that are that are \emph{relevant} (or that are used) in some way by the query to generate the output.}
...
It takes the form of a polynomial, called \emph{provenance polynomial}, where the variables are taken from the set $X$ of identifiers of the tuples (provided that each tuple in $I$ has an identifier) and the coefficients are drew from the set of natural numbers $\mathbb{N}$. \rtwo{This semiring is commonly referred as $\mathbb{N}[X]$ in the literature.}
% SOME FORMALISM
...
A formal study of causality was initiated in \cite{Halpern2013Causality,ChocklerH04} and later expanded by \citet{MeliouGMS11} to define the causes of answers and non-answers to queries. Causality is related to the provenance of a query result such as lineage and adds more information to it.

In the following we define causality and responsibility as done in \cite{MeliouGMS11}, but differently from \cite{MeliouGMS11}, we only focus on answers of a query, and not on non-answers, since they are not relevant in our context.
Let 
...
Computing the causality of tuples is NP-complete in general~\cite{EiterL02}, but it has been proven \citet{MeliouGMS11} that the causality of conjunctive queries is PTIME.


The notion of \emph{responsibility} was first defined in \cite{ChocklerH04}, and it measures the degree of causality as a function of the size of the smallest contingency set. It allows us to rank the tuples in a lineage based on their degree of causality in generating the output. 
...
As can be seen, a counterfactual cause will have the maximum responsibility of $1$, while the larger the minimum contingency of an actual cause, the smaller its responsibility since there are alternatives to  guarantee the presence of the answer $\bar{a}$.
