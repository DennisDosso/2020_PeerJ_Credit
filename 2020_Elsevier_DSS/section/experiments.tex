...
\rone{The polynomials were generated as follows (every time we use the word ``randomly'', we mean that we used a uniform distribution): first, the number of monomials composing the polynomial is decided choosing randomly a number between 1 and 6.  Then, we randomly choose a tuple from the tables \texttt{family}, one from the table \texttt{contributor2family} and one from table \texttt{contributor} that are used as the monomial's variables. We then choose a coefficient for this monomial (between 1 and 3) and an exponent for each tuple (between 1 and 4). For the next monomial, then, we decide if we want to keep the same tuple from the table family as first tuple of the new monomial. To do so, we generate a random number between 0 and 1. If the number is above $0.2$, we change the family tuple.}

An example can be found in Figure~\ref{fig:syntheticDistributions}, which shows a sample synthetic provenance polynomial (the how-provenance), the corresponding why-provenance, lineage, \rtwo{and the causality of the tuples of the lineage, together with their responsibility}.  The resulting credit distribution for each DS is also shown. 
...
Responsibility, on its part, can be seen as an enrichment of the information brought by lineage. It creates a ranking among lineage's tuples describing the importance of their role in generating the output. As such, the responsibility-based DS gives more credit to $f_1, f_5, c2f_17$ and $c_18$ due to their higher responsibility values. This notion of ``importance'' is connected to their corresponding minimal contingency sets. For example, $f_1$ has as minimal contingency set (one of the many) $\{f_5\}$, with cardinality 1. On the other hand, $c_1$ has, as minimal contingency set (one of the many) $\{f_5, c_2\}$, with cardinality 2. This means that $c_1$ is ``less important'' of the tuples with minimal contingency sets of lower cardinality, and this is reflected on the different quantity of credit being distributed.} 

Despite being synthetic, these provenance polynomials may actually be obtained from queries.  The polynomials can be obtained by any nested query with join and union operations that use the same tuple multiple times (in which case the exponents are bigger than $1$), and the same combination of operations more than once (in which case the coefficients of monomials are bigger than $1$). 
...
We note that the lineage-based DS gives an average credit of $3.92$ to each tuple in the table, while the DS based on why-provenance assigns $4.18$, how-provenance $4.18$, \rtwo{and the one based on responsibility $4.13$}. 
Moreover, lineage distributed a total of about $3121$ units of credit to the \texttt{family} table, why-provenance $3333$, how-provenance $3331$, while \rtwo{responsibility assigned $3290$}. 
...
\rtwo{Moreover, it can be seen that tuples 1 and 7 receive a higher quantity of credit when how-provenance is exploited with respect to the overs, showing how this provenance behaves differently from the others in this context}.
...
\rtwo{We also note that the responsibility-based distribution confirms its trend of being similar to why-provenance, although not completely identical. This is more evident at step 10K, where tuple 7 is slightly less rewarded using responsibility ($6.12$) with respect to why-provenance ($6.24$). 
This is due to the fact that tuple 7 had, among some of the polynomials being used for the experiments, a high responsibility but it did not appear in al witnesses. This changed slightly the distribution.}
