...
The fourth set of experiments shows how the DSs compare to traditional citations in giving credit to data curators using both real and synthetic queries. We close by discussing the relative execution times of the three strategies.
...
%\scream{Revised, check that it is correct.  - yes}


\scream{Only introduce detail if you use it later... why do people need to know the names of the families, or how many there are?  I've "eaten" it again, but if you insist you can pull it out again.}
...
Therefore, the same queries can be used to build all of the target family pages by simply changing the family id used in the query (for example, in Figure \ref{figure:family_structure}, it is 3).
...
We note that, however, there is  consistency in how credit is distributed over the tuples between the three strategies, i.e. tuples that are highly rewarded by one strategy are also highly rewarded by the others. This shows that the three DS consistently reward certain tuples more than others. 
...
To better understand this difference, in the next subsection we consider the accrual of credit over time.  In doing so, we will focus on the ten tuples shown within the large blue rectangles in Figure \ref{fig:comparison}.  Each small rectangle within a large blue rectangle is a tuple, and we number them from 1 (top) to ten (bottom).  
...
are those  highlighted in the blue boxes of Figure \ref{figure:comparison_on_synthetic_polynomials_2} from the \texttt{family} table.  
...
The queries used are the same as the experiment reported in the previous section. The range of credit in each map goes from 0 (no credit) to 8 (the maximum quantity of credit reached on one of the tuples of the considered window at the ``snapshot'' with 10K queries). The color hue of the legend, as can be seen, still ranges from $0$ to $9.5$.

\scream{I feel that this section is going into too much detail.  We should just get the big idea across.  I "eaten" most of this text and tried to pull out the highlights. The other problem is we can't really say why tuples wax and wane in overall credit -- it depends on the queries, which we don't go into (or want to go into).}

By the end of 1K queries, credit differentials between tuples as well as between strategies can be seen.  For example, tuple 4 has the highest value of credit overall, with the highest value coming from the why-provenance DS.   This trend continues to the end of 2k queries, at which point tuple 5 also gains in credit. 
...
While the relative value of credit``positions" of tuples within a DS strategy depends on the exactly what queries are being executed, the important thing to notice is the difference between the DSs over time:  Overall, lineage gives far less credit to tuples in the \texttt{family} table than the other two strategies since credit is  shared with tuples in other tables.
...
The differences between the why- and how-provenance-based DSs are also relatively minor (about plus or minus 0.2 out of 9.5) in most cases. However, there are certain situations when the role of a tuple is particularly critical in a query, and in this case the difference in the value of credit assigned is notably higher for how-provenance. 
\scream{Higher for how-provenance?}
...
However, a DS based on why- or how-provenance may be better if the queries are complex, since they reward tuples more that have a critical role in generating the output. %These two DS are sensitive than the DS based on lineage (and the DS based on how-provenance is more sensible than the one based on why-provenance). 
...
In particular, these two DSs may be useful for finding ``hotspots'' in the database based on the role of tuples, with the how-provenance-based DS being preferable if a higher sensitivity to the role of a tuple in queries is required. 
...
Each target family page in GtoPdb has a list of curators, representing the people who worked on the page and who can be considered as co-authors of the page. The list of co-authors of a page can be obtained using the last query shown in Figure \ref{figure:family_structure}. 
\eat{As set of authors, we consider all the curators of one target family in GtoPdb. }
\scream{This is confusing because you switch between "pages" and "data".  I've rewritten, but it may be wrong.}
Each time a target family page is cited, we assign one {\em citation} to each author associated with the page.  The authors also receive {\em credit} in the amount assigned to the data returned by the query used to construct the webpage, equally divided between the authors of the webpage.

\eat{Each time data is cited, we assign one citation to each author associated with the page. Each time some data, e.g. a tuple, receives credit, we equally divide and transitively assign that quantity of credit to the authors of that data.}
...
Figure \ref{figure:2_radars}.a, shows the top 20 authors, ordered in a clockwise direction based on citations, whereas Figure \ref{figure:2_radars}.b orders the authors ordered by credit. 
%\scream{what does that mean?  and it seems that there is not a large overlap in the authors in fig. a and b}
As we see from Figure \ref{figure:2_radars}.a, the top 20 authors in terms of citations do not have the highest values of credit. Similarly, Figure \ref{figure:2_radars}.b shows that the authors with the highest values of credit do not necessarily have the largest number of citations.
From this it can be seen that the two sets of ``top authors'' using these two metrics are not the same, although there is partial overlap (for example, authors 1774, 475, and 4012). 

\scream{I don't understand what you are trying to say here.  Rewrote, putting old in eat in case I am wrong.}
From this figure we can see that there is a significance difference between citations and credit:  Authors with the highest number of citations may not receive the highest value of credit. Conversely, authors with the highest value of credit may not have the highest number of citations. \eat{This is due to the fact that certain citations are more ``valuable'' in terms of credit, i.e. an author receive more credit from his/her citations, even if fewer, than other authors. This, in turns, happens because certain citations generate more credit than others. 
...
For example, author 536 has the highest value of credit, but is not even in the top 20 authors in terms of citations. This means that he receives much more credit from his relatively few citations than author 475, who receives the most citations.
That is, the data underlying certain webpages is more ``valuable'' in terms of credit than a citation to the webpage.  

\scream{I also didn't quite understand this, reworded.}
...
Part of this is due to the experimental setup:  Each output tuple carries a credit $1$, and there can be many tuples used to generate a webpage.  Thus a webpage that is created from more tuples will have a higher credit value than one created from fewer tuples. Furthermore, authors (curators/contributors) who collaborated with fewer people will receive a biggest share from the equally divided credit.  However, all authors will receive a citation of one.

Credit distribution therefore rewards authors differently than traditional citations: An author who has curated larger quantities of cited data, and collaborated with fewer co-authors, will receive larger quantities of credit. Thus, credit can reward them for their larger ``contribution'' to the database. 
...
\scream{Perhaps omit this paragraph? I reworded a bit, but it is mysterious what other strategies you are talking about.  Is this more more "future work"?}
\eat{
...
We produced $100$, $1K$, and $10K$ synthetic polynomials, as described in Section \ref{sec:synth_queries}, and distributed credit through them to data. 
Since these polynomials are composed by tuples randomly selected from three tables, they usually correspond to very large set of authors that in reality did not collaborate. 
...
To simplify the assignment of data curators to the hypothetical queries generating these polynomials, we created $20$ ``synthetic'' authors, and we randomly assigned one author to each tuple in the database. The authors receive ``blocks'' of consecutive tuples, with each block of a size varying between $10$ and $40$ to simulate different quantities of ``work'' performed by an author. 
Every time an author appears as curator of one or more tuples used in a polynomial, we assigned him one citation.  
He also receives three kinds of credit, the ones assigned to his tuples through the three different DSs.

Figure \ref{figure:3_radars} reports the three radar plots that are a consequence of the distribution of credit and citations performed and described above. 
Figure \ref{figure:3_radars}.a reports the values obtained with 100 polynomials, showing the normalized values of the citation and types of credit assigned to each author. 
As we see, given the synthetic nature of these queries, the correlation between the number of citations and the quantity of credit assigned to the authors appears to be a much stronger with respect to the case with the real-world queries of Figure.\ref{figure:2_radars} (the linear correlation between the citation number and all three types of credit is always above 0.95 with p values in the order of 1e-11).
...
Similar observations can be made for Figure \ref{figure:3_radars}.b and \ref{figure:3_radars}.c, where we kept the order of authors of Figure \ref{figure:3_radars}.a.
...
It is worth pointing out that, when scaling up to $1K$ and $10K$ polynomials, the distributions performed via why-provenance and how-provenance become almost equivalent. We can note that, although not exactly overlapping, the values of credit assigned to the authors by those DS become quite similar with these higher quantities of polynomials, suggesting a sort of equivalence between the two DSs in this case, at least in the task of rewarding authors (the linear correlation for the values of Figure \ref{figure:3_radars}.c is more than 0.99 with a p-value of 1.32e-32). This is consistent to what we already pointed out in Figure \ref{figure:comparison_on_synthetic_polynomials_2}.
