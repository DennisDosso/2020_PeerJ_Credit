...
\rrtwo{We focus on SQL SPJ queries and bag semantics}, and we define three new distribution strategies, the first one based on how-provenance, the second based on the concept of responsibility, and the fourth on the Shapley value. 
...
We also propose other two DS based on the concepts of responsibility~\cite{MeliouGMS11} and the Shapley value~\cite{LivshitsBKS20,DFKM22}. \rrtwo{We work with bag semantics, since our work focuses on the real-world scenario of SQL queries.}
...
We now describe the notion of how-provenance, causality, responsibility, and the Shapley value function. 
...
How-provenance was first defined in \citep{howProvenanceGreen} to capture this information about how the source tuples are used using a \emph{semiring} algebraic structure.
...
% SOME FORMALISM
\rrtwo{In the following, the (largely standard) notation used in \cite{CheneyProvSurvey} is applied. }
\rrtwo{Let $\textbf{D}$ be a finite domain of data values $\{d_1, \dots, d_n\}$ and $\mathcal{U}$ a collection of \emph{field names} (also attribute names). We use the symbols $U, V$ to indicate finite subsets of $\mathcal{U}$.}

\rrtwo{A record or tuple $t$ is a function $U \mapsto \textbf{D}$, written as $(A_1:d_1, \dots, A_n:d_n)$. A tuple assigning values to each field name in $U$ is called $U$-tuple. 
...
The how-provenance of an output tuple is a function $\mathcal{H} = How(Q, I, o)$ that returns a polynomial in $\mathbb{N}(TupleLoc)$ called \emph{provenance polynomial}. The following definition was adapted from \cite{CheneyProvSurvey}.}
...
To better show how the how-provenance highlights the ways in which tuples are used by a query, we consider another query over GtoPdb, \texttt{Q2}, that asks for the families of type \texttt{gpcr} that have as contributor a researcher located in the UK:
...
As we can see, the how-provenance highlights through the use of exponents the fact that $f_1$ is used twice: first in the join of the inner query, and second in the join of the outer query. 
...
Figure \ref{figure:distributions_differences} shows how the DS based on how-provenance behaves on two different polynomials. Respectively, the one obtained from query \texttt{Q1} (subfigure a) and the one obtained from query \texttt{Q2} (subfigure b). 
...
\rrtwo{As we see, in case of Figure \ref{figure:distributions_differences}.a, tuple $f_1$ receives credit $1/3$ (the other tuples receive each credit $1/6$), while in Figure \ref{figure:distributions_differences}.b tuple $f_1$ receives credit $1/2$ (and the other receive credit $1/8$).}
% The DS based on the provenance polynomial gives credit $1/2$ to $f_1$, and $1/8$ to the other tuples.
This is reasonable since \texttt{Q2} relies on $f_1$ even more than \texttt{Q1} does, \rrtwo{and shows how how-provenance is sensitive to the tuples' role in a query}.
...
\rrtwo{In the following definition we call $L$ the \emph{lineage set} of a tuple $o \in Q(I)$, i.e., the set of all and only the tuples in the database instance I that are used by query $Q$ to produce the output tuple $o$.}
...
The reason why credit distribution is the same for all strategies is that the queries are all simple SPJ queries, which use each table only once and do joins on key attributes \rrone{(there are always 1-to-1 joins)}. 
Under these conditions, each tuple of the output presents: (i) a how-provenance that is a single monomial with coefficient one and exponent one in each variable; (ii) all tuples are counterfactual causes when considering responsibility, thus they have responsibility 1, and (iii) all tuple have the same importance in the production of the output tuples according to their Shapley value.
...
As we see the distribution based on how-provenance is the one that correlates less with the other twos, while it seems that the DSs based on responsibility and the Shapley value are more correlated.
\rrone{This may be explained considering the fact that, while how-provenance captures more the way in which the tuples are used, while the other two strategies are more concerned with the importance of the tuples in lineage of the query (responsibility) and the role that the tuples have in the query seen as a coalition game (Shapley value).  }

% The provenance-based approaches certainly satisfy 1and 43 and I believe that at least why- and how-provenance satisfy 2.  However, they don’t satisfy linearity (Nave gave me a counterexample for this).
...
\rrtwo{Considering the three heat-maps presented in Table \ref{figure:comparison_on_synthetic_polynomials_2}, it is immediately evident that they are still similar, i.e., certain tuples are rewarded more than others due to their importance on the execution of queries. 
However, upon closer inspection, it is possible to see more clearly that they are actually behaving differently, with certain tuples rewarded more with one strategy rather than with the others.}
...
We note that the how-provenance-based DS gives an average credit of $4.18$ to each tuple in the table, while the one based on responsibility $4.13$, and the one based on the Shapley value $4.40$. 
% Moreover, lineage distributed a total of about $3121$ units of credit to the \texttt{family} table, why-provenance $3333$, 
Moreover, how-provenance distributed a total of about $3331$ units of credit to the  \texttt{family} table, while \rtwo{responsibility assigned $3290$, and the Shapley value $3505$. Thus, the Shapley value is the method that accumulates the highest quantity of credit in this table.} 
...
It distributes credit to the tuples based on their role in the queries and, in particular, the tuples that were used more and in many different ways will be the ones receiving more credit.}
\rrtwo{The distributions based on responsibility and the Shapley value are more concerned with the importance of the single tuples in the generation of the output, and thus they may be different.}
...
In particular, these three DSs may be useful for finding ``hotspots'' in the database based on the role of tuples. The preference of one over the others depends on the type of sensitivity to the role of a tuple in queries that is required by the context. 
...
Each plot shows the top 20 authors in terms of citations (hence the authors and clockwise ordering is the same in each of the plots), and additionally shows the the normalized values of citation (blue line), 
...
\label{sec:time}
\rrtwo{We studied the time required to compute the how-provenance and the other forms of information used in the paper, and to distribute the credit with the three proposed DSs. We first computed the how-provenance of the real queries using the code provided in \cite{Wu2020ProvCite} on the GtoPdb database, obtaining a total of $58037$ polynomials. 
Responsibility and Shapley values of the output tuples of the queries were computed starting from these polynomials.  }
...
\begin{table}[]
    ...
    \hline
         ...
         \hline
         synthetic polynomials & - & $1.48\pm0.05$ & $39.79\pm2.87$ \\
         \hline
    \end{tabular}
    \label{tab:execution_times}
\end{table}

\rrtwo{Table \ref{tab:execution_times} reports the average times required to compute how-provenance, responsibility, and Shapley values of one tuple of the outputs, both in the case of real queries and in the case of synthetic polynomials (here, we consider all $10000$ produced synthetic polynomials). All times are reported in milliseconds. 
More precisely, the time reported in the table to compute how-provenance is the one reported using the code provided in \citep{Wu2020ProvCite}, while the times reported for responsibility and Shapley value are the one needed to derive them from the how-provenance of the single tuples. In these last two cases the total time is given by the sum of the time to compute the how-provenance and the time to convert it into responsibility/Shapley value. 
The synthetic polynomials were produced directly and not from synthetic queries, thus there is no time for the computation of the how-provenance in that case.}

\rrtwo{We notice from this table that the cost to compute responsibility is lower than the one to compute the Shapley value. In our implementation, in fact, we follow the definition of Shapley value and, as such, we compute the power set of the lineage of the output tuples. This requires, on average, a bigger computation time. In particular, in the case of synthetic polynomials, this time increases since, on average, the lineage is bigger.}
...
These studies show that operations such as inner-/outer-joins and projections occur in a significant number of queries.  Therefore how-provenances may become quite complex in certain cases and provide a distribution of credit that is significantly different from the one obtained with lineage. }
...
From a computational complexity standpoint, all three DSs are similar since we focused on SPJ queries. \rrtwo{Our experiments in Section \ref{sec:time} showed the bigger overhead connected to the Shapley value, but those results suffer by the limit of the test environment: a relatively small database and a rather na\"ive algorithm to compute responsibility and Shapley values.} 

Going beyond SPJ queries, \citet{howProvenanceGreen} proposed the provenance semiring framework for SPJRU (Select, Project, Join, Rename, and Union queries), and \citet{AmsterdamerDT11} showed how to extend the framework to aggregate queries.
...
\rrtwo{We can conclude that, given the available algorithm to compute how-provenance, responsibility and Shapley value, all the proposed DSs are feasible, even with bigger query logs and in particular for scientific databases, whose sizes are usually not a challenge for the algorithms. One may prefer one over the other due to the properties of the information at its base. Thus, if a user prefers to simply know whose tuples were used in the query, they can use lineage as described in \cite{dosso2020data}; if they prefer to weigth the DS on how the tuples were used, the how-provenance-based DS may be chosen; if they want to consider the ranking of the triples as expressed by their responsibility, or to exploit the properties of the Shapley value, the other two DSs presented in the paper may be taken into consideration.}
...
\rrone{For this the notion of \emph{game provenance} $\Gamma(x)$ is introduced in ~\cite{Kohler2013}. In spirit, is similar to the notion of how-provenance but it works for games and explains the value (won, lost, or drawn) for a position $x$.}

\rrone{Let us consider the function $\lambda : V \times V \rightarrow \{g, r, y\}$. This functions defines a color for a subset of edges from $M$. A move can either be \emph{winning} ($g$, moving from a winning node to a losing node), \emph{delaying} ($r$, moving from a losing node to a winning node), or \emph{drawing} ($y$, moving from a draw to a draw). Given a solved game $G^\gamma$, its game provenance is the $\lambda-$colored subgraph $\Gamma$ of $G^\gamma$ by the function $\lambda$. In other words, the game provenance is the subgraph of the game graph composed by the nodes together with their tags and the edges, together with the tags given by $\lambda$.}
...
\rrone{\paragraph{Query Evaluation Games} Since any First-Order (FO) query $\varphi(\bar{x})$ on an input database instance $D$ can be expressed as a non-recursive Datalog$^\neg$ (Datalog with negation) program $Q_\varphi$, it is observed in \cite{Kohler2013} that the evaluation of $Q(D)$ can be seen as a game between players I and II who argue whether an atom $A \in Q(D)$. 
...
Given a positive query $Q$ and an atom $A(\bar{x}) \in Q(D)$, it is possible to convert its provenance graph to an \emph{operator tree}, i.e., a special case of game graph where each node belongs to one of three classes: \emph{rule nodes}, \emph{goal nodes}, and \emph{relation/tuple nodes}  (the procedure to build these trees is described in~\cite{Kohler2013}). These trees are particularly interesting since they can be used to explain the presence (why-provenance) or absence (why-not provenance) of a fact in the output of a query.}

\rrone{\paragraph{Game Provenance and How-provenance} It is shown in \cite{Kohler2013}  that it is possible to obtain the how-provenance of output facts in $Q(D)$ from their Game Provenance (the provenance polynomials for the result of a positive query can be ``read out'' from a provenance game~ \cite{Lee2018PUG}). 
% Not only that, but it is also shown that game provenance yields meaningful explanations for $why-not$ questions (i.e., why an atom is not in $Q(D)$.
}

\rrone{ 
% Consequently, it is possible to perform credit distribution for positive queries starting from their game provenance and deriving their how-provenance.
We note that by using game provenance it is possible to expand the task of credit distribution to FO queries (i.e., non-recursive Datalog queries with negation). 
In fact, new DSs based on the information provided by the operator trees of queries can be devised, thus dealing also with FO queries.
For example, since the leaves of an operator tree are the tuples of the database being used by the query, the new DSs can be based on the tree topology,  propagating the credit as a flux through its nodes and edges. }

\rrone{Moreover, as shown in \cite{Lee2018PUG}, these operator trees can also be used for why-not provenance, i.e., to explain the \emph{absence} of a fact from the query output. This can potentially open up new applications of data credit, i.e., the generation of credit to ``missing'' facts in the output of queries, aimed to assign  credit to ``missing'' facts in the database instance whose absence is responsible for the missing result. 
This, in turn, may be useful for the administrators to understand what is missing in their database to correctly answer the most searched queries, and decide whether and where to allocate the necessary resources to create/add those data if possible.}
...
\rtwo{While the DS based on lineage presented in \cite{dosso2020data} rewards all the tuples used by a query equally, the strategy based on  responsibility give more credit to  tuples that are more critical to the query.
