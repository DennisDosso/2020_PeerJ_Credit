...
\rrtwo{As we see, in case of Figure \ref{figure:distributions_differences}.a, tuple $f_1$ receives credit $1/3$ (the other tuples receive each credit $q/6$), while in Figure \ref{figure:distributions_differences}.b tuple $f_1$ receives credit $1/2$ (and the other receive credit $1/8$).}
...
\rtwo{Table \ref{table:kendall_tau} reports the pairwise Kendall $\tau$ correlation values\footnote{The Kendall's $\tau$ coefficient is a statistic used to measure the ordinal association between two measured quantities~\cite{Kendall1938new}. Intuitively, it is high between two variables when observation have a similar rank.} for the three DSs computed on the \texttt{family} table. 
...
\rrone{This may be explained considering the fact that, while how-provenance captures more the way in which the tuples are used, while the other two strategies are more concerned with the importance of the tuples in the query.}
}
